...
| 2 | ~~非 macOS 上的缩略图方案~~ **已实测，仅剩 Linux 替代方案待定** | §6.5 与验收 #16 | **macOS 已坐实**：24 张全库图用 `sips -s format jpeg -Z 480` 生成缩略图共 **0.96 MB**（原始 58MB），验收 #16 的 3MB 预算达标且余量充足。`sips` 是 macOS 独有，Linux 上仍需替代方案或明确接受该项不达标 |
| 3 | ~~`sips` 能否产出 WebP~~ **已实测，结论：不能** | §6.3 模板缩略图格式 | `sips -s format webp` **写入失败**，WebP 确认只读。§6.3 已改为 JPEG。同时实测：`sips -Z 480` 不加 `-s format jpeg` 时输出的是 **227 KB 的 PNG**（`file` 报 `image/png`），加上后为 **44 KB** —— **5.2 倍差**，比原估的 4.5 倍更大 |
...
未核实前，**第 2 期可以动工**（它不依赖上述任何一条）；第 3 期依赖第 1 条。**第 4 期原本依赖第 2、3 条，现已实测放行**——缩略图格式定为 JPEG、macOS 体积实测 0.96 MB 达标，仅 Linux 替代方案仍开放（不阻断 macOS 上的交付）。
